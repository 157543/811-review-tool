...
- **划重点原件缺项已解决（2026-09-15）**：新增《华南理工大学章节及课后习题划重点(1).pdf》已登记到 `source_notes/manifest.local.json`，当前资料目录为16个文件、5个PDF共1,239页；这不是对全部PDF内容的审核。KEY_EXERCISE从“缺清单而无法核验重点资格”更新为“82项清单资格、教材定位及主题映射已核对，可开展逐题审核”。`source_notes/index/key-exercises-phase1.md` 保留原注和适用边界；答案审核仍未开展，没有任何正式题或Question审核状态变更。
- **新增来源差异只报告，不调配额**：个别目录笔误、习题号与小节号混淆、相关函数/希尔伯特变换等超出当前明确知识点、部分习题深度超出阶段一，见 `source_notes/conflicts/key-exercises-increment-2026-09-15.md`。305题配额保持不变。3.4、4.7、5.7未从这82题获得直接主题映射，可继续使用已有教材正文等来源，不应删除配额。
...
题库进入正式发布同时要求整题 verified、来源已核验且无未解决冲突。形式校验成功只能说明结构合格，不能证明答案数学正确或干扰项有效。
