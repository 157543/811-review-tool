...
import { EMPTY_FILTERS } from '../../src/domain/models';
import type { Attempt,Mistake,Question,Session,Settings } from '../../src/domain/models';
import { buildLearningDashboard,getDueState } from '../../src/domain/progress';
import { sampleQuestions } from '../../src/domain/sampling';
import release from '../../data/questions/release.json';

const now=new Date('2026-09-16T00:00:00.000Z');
const questions=(release as {questions:Question[]}).questions.slice(0,5);
...
const settings:Settings={id:'primary',current_chapter:5,grading_mode:'immediate',timezone:'Asia/Shanghai',sampling_rule_version:'phase1-default-v1',schedule_rule_version:'simple-spaced-v1',schedule_policy_id:'simple-spaced-v1-1-3-7',review_intervals_days:[1,3,7],chapter_weights:{'1':1,'2':1,'3':1,'4':1,'5':1},recent_window_days:14,family_cooldown_count:15};
...
describe('learning dashboard',()=>{
  ...
  it('keeps a due review ahead of the recent-family cooldown',()=>{
    const result=sampleQuestions({questions,mistakes:[mistake(0,'WEAK','2026-09-15T00:00:00.000Z')],attempts:[attempt],reasonAnnotations:[],settings,filters:EMPTY_FILTERS,mode:'today',count:1,now,random:()=>.5});
    expect(result.questions[0].id).toBe(questions[0].id);expect(result.bucketCounts.due).toBe(1);
  });
});
