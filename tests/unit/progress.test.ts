...
import fixtureQuestions from '../fixtures/question-bank';
import { EMPTY_REASON_DISTRIBUTION } from '../../src/domain/models';
import type { Attempt,Mistake,Session } from '../../src/domain/models';
...
function mistake(questionIndex:number,status:Mistake['status'],due:string|null):Mistake {
  const question=fixtureQuestions[questionIndex];
...
describe('learning dashboard',()=>{
  ...
  it('summarizes chapter progress and the latest completed session',()=>{
    const result=buildLearningDashboard(fixtureQuestions,[mistake(0,'WEAK','2026-09-15T00:00:00.000Z')],[attempt],[session],now);
    expect(result.dueToday).toBe(1);expect(result.practiced).toBe(1);
    expect(result.chapters.find(item=>item.chapter===fixtureQuestions[0].chapter)?.practiced).toBe(1);
